...
main = if testserverid == -1
  then TIO.putStrLn
    "ERROR: modify the source and set testserverid to your serverid"
...
startHandler = do
  let activity =
        def { activityName = "ping-pong", activityType = ActivityTypeGame }
  ...
  either
    ...
    (\chans -> forM_
      ...
      (\channel -> restCall $ R.CreateMessage
        (channelId channel)
...
newExampleSlashCommand =
  createApplicationCommandChatInput "subtest" "testing out subcommands"
    >>= \d -> Just $ d
          { createApplicationCommandOptions = Just
              ...
              [ ApplicationCommandOptionSubcommandGroup
                ...
                [ ApplicationCommandOptionSubcommand
                    ...
                    [ ApplicationCommandOptionValueString
                      ...
                      "two options"
                      True
                      (Right [Choice "green" "green", Choice "red" "red"])
                    , ApplicationCommandOptionValueInteger "oneintinput"
                                                           "choices galore"
                                                           False
                                                           (Left False)
                                                           Nothing
              ...
              , ApplicationCommandOptionSubcommandOrGroupSubcommand
                $ ApplicationCommandOptionSubcommand
                    ...
                    [ ApplicationCommandOptionValueString
                        ...
                        "two options"
                        True
                        (Right [Choice "yellow" "yellow", Choice "blue" "blue"])
                    ]
              , ApplicationCommandOptionSubcommandOrGroupSubcommand
                $ ApplicationCommandOptionSubcommand
                    "sndsubcmd"
                    "the second subcommand"
                    [ ApplicationCommandOptionValueBoolean "trueorfalse"
                                                           "true or false"
                                                           True
                    , ApplicationCommandOptionValueNumber "numbercomm"
                                                          "number option"
                                                          False
                                                          (Left True)
                                                          (Just 3.1415)
                    ...
                    , ApplicationCommandOptionValueInteger
                      ...
                      False
                      (Right
                        [Choice "one" 1, Choice "two" 2, Choice "minus 1" (-1)]
                      )
                      (Just $ -1)
                      (Just $ -2)
                    , ApplicationCommandOptionValueInteger
...
exampleSlashCommand =
  createApplicationCommandChatInput "test" "here is a description" >>= \cac ->
    return $ cac
...
exampleInteractionResponse
  :: InteractionDataApplicationCommandOptions -> InteractionResponse
exampleInteractionResponse (InteractionDataApplicationCommandOptionsValues [InteractionDataApplicationCommandOptionValueString { interactionDataApplicationCommandOptionValueStringValue = s }])
  = interactionResponseBasic
    (T.pack $ "Here's the reply! You chose: " ++ show s)
exampleInteractionResponse _ = interactionResponseBasic
...
eventHandler event = case event of
  MessageCreate m -> when (not (fromBot m) && isPing m) $ do
    ...
    let
      opts :: R.MessageDetailedOpts
      opts = def
        { R.messageDetailedContent         =
          "Here's a more complex message, but doesn't ping @everyone!"
        , R.messageDetailedTTS             = True
        , R.messageDetailedAllowedMentions = Just $ def
                                               { R.mentionEveryone    = False
                                               , R.mentionRepliedUser = False
                                               }
        , R.messageDetailedReference       = Just $ def
                                               { referenceMessageId = Just
                                                                        $ messageId m
                                               }
        }
    ...
    let
      ...
      opts' = def
        ...
        , R.messageDetailedComponents = Just
          [ ComponentActionRowButton
            [ ComponentButton "Button 1"
                              False
                              ButtonStylePrimary
                              "Button 1"
                              (Just (mkEmoji "🔥"))
            , ComponentButton "Button 2"
                              True
                              ButtonStyleSuccess
                              "Button 2"
                              Nothing
            , ComponentButtonUrl "https://github.com/aquarial/discord-haskell"
                                 False
                                 "Button 3"
                                 Nothing
            ]
        ...
        , R.messageDetailedEmbeds     = Just
          [ def
            ...
            , createEmbedImage       = Just
              (CreateEmbedImageUrl
                "https://media.discordapp.net/attachments/365969021083975681/936055590415921172/Warning.png"
              )
            , createEmbedColor       = Just DiscordColorLuminousVividPink
            , createEmbedAuthorIcon = Just (CreateEmbedImageUpload exampleImage)
            }
          , def { createEmbedTitle       = "a different Title"
                , createEmbedDescription = "another desc"
                }
          ]
        }
  ...
  Ready _ _ _ _ _ _ (PartialApplication i _) -> do
    vs <- mapM
      (maybe (return (Left $ RestCallErrorCode 0 "" ""))
             (restCall . R.CreateGuildApplicationCommand i testserverid)
      )
      [ exampleSlashCommand
      , exampleUserCommand
      , newExampleSlashCommand
      , createApplicationCommandChatInput "modal" "modal test"
      ]
    liftIO
      (putStrLn $ "number of application commands added " ++ show (length vs))
    acs <- restCall (R.GetGuildApplicationCommands i testserverid)
    case acs of
      Left r -> liftIO $ print r
      Right ls ->
        liftIO $ putStrLn $ "number of application commands total " ++ show
          (length ls)
  InteractionCreate InteractionComponent { interactionDataComponent = cb@InteractionDataComponentButton { interactionDataComponentCustomId = (T.take 3 -> "ttt") }, ..}
    -> case processTicTacToe cb interactionMessage of
      [r] -> void
        (restCall
          (R.CreateInteractionResponse interactionId
                                       interactionToken
      ...
      r : rs ->
        void
            (restCall $ R.CreateInteractionResponse
              interactionId
              interactionToken
              (InteractionResponseUpdateMessage r)
            )
          >> mapM_
               ( restCall
               . R.CreateFollowupInteractionMessage interactionApplicationId
                                                    interactionToken
               )
               rs
      _ -> return ()
  InteractionCreate InteractionApplicationCommand { interactionDataApplicationCommand = InteractionDataApplicationCommandUser { interactionDataApplicationCommandName = nm, interactionDataApplicationCommandTargetId = uid, ..}, ..}
    -> void $ restCall
      (R.CreateInteractionResponse
        interactionId
        interactionToken
        (interactionResponseBasic $ "Command " <> nm <> T.pack
          (" selected user: " ++ show uid)
        )
      )
  ...
  InteractionCreate InteractionApplicationCommand { interactionDataApplicationCommand = InteractionDataApplicationCommandChatInput { interactionDataApplicationCommandName = "subtest", interactionDataApplicationCommandOptions = Just d, ..}, ..}
    -> void $ restCall
      (R.CreateInteractionResponse
        interactionId
        interactionToken
        (interactionResponseBasic
          (  T.pack
          $  "oh boy, subcommands! welp, here's everything I got from that: "
          <> show d
          )
        )
      )
    ...
    -> void $ restCall
      ...
      $ InteractionResponseChannelMessage
          ((  interactionResponseMessageBasic
           ...
           <> interactionDataComponentCustomId
           )
            { interactionResponseMessageFlags = Just
      ...
      (do
        ...
        restCall
          (R.CreateFollowupInteractionMessage
            ...
            interactionToken
            (interactionResponseMessageBasic
                (T.pack $ "oh dear, select menu. thank you for waiting" <> show vs
                )
              )
              { interactionResponseMessageEmbeds = Just
                [ def
                    { createEmbedTitle       = "Select menu title"
                    , createEmbedDescription =
                      "Here is the select menu embed desc"
                    ...
                    , createEmbedAuthorIcon  = Just
                      (CreateEmbedImageUpload exampleImage)
                    }
                ]
              }
          )
      )
  InteractionCreate InteractionApplicationCommandAutocomplete { interactionDataApplicationCommand = InteractionDataApplicationCommandChatInput { interactionDataApplicationCommandName = "subtest", interactionDataApplicationCommandOptions = Just _, ..}, ..}
    -> void
      (restCall $ R.CreateInteractionResponse
        interactionId
        interactionToken
        (InteractionResponseAutocompleteResult
          (InteractionResponseAutocompleteInteger [Choice "five" 5])
        )
      )
  ...
  InteractionCreate i@InteractionModalSubmit { interactionDataModal = idm } ->
    void $ restCall
      (R.CreateInteractionResponse
        (interactionId i)
        (interactionToken i)
        (interactionResponseBasic (T.pack (show idm)))
      )
  _ -> return ()

processTicTacToe
  :: InteractionDataComponent -> Message -> [InteractionResponseMessage]
processTicTacToe (InteractionDataComponentButton cid) m =
  case messageComponents m of
    Nothing ->
      [ interactionResponseMessageBasic
          "Sorry, I couldn't get the components on that message."
      ]
    (Just cs) ->
      let newComp = newComp' cs
      in
        ((interactionResponseMessageBasic
           (  "Some Tic Tac Toe! Player "
           <> (if '0' == T.last (messageContent m) then "1" else "0")
           )
         )
            { interactionResponseMessageComponents = Just
              ((if checkTicTacToe newComp then (disableAll <$>) else id) newComp)
            }
          )
          : [ interactionResponseMessageBasic
                ("Player " <> T.singleton player <> " has won!")
            | checkTicTacToe newComp
            ]
 where
  player   = T.last (messageContent m)
  newComp' = updateTicTacToe (Just (cid, '0' == player))
  disableAll (ComponentActionRowButton cs) =
    ComponentActionRowButton
      $   (\c -> c { componentButtonDisabled = True })
      <$> cs
  disableAll c = c
processTicTacToe _ _ =
  [interactionResponseMessageBasic "Sorry, I couldn't understand that button."]

checkTicTacToe :: [ComponentActionRow] -> Bool
checkTicTacToe xs = checkRows unwrapped || checkRows unwrappedT || checkRows
  [diagonal unwrapped, diagonal (reverse <$> unwrapped)]
 where
  checkRows =
    any (\cbs -> all (\cb -> cb == head cbs && cb /= ButtonStyleSecondary) cbs)
  unwrapped =
    (\(ComponentActionRowButton cbs) ->
        (\ComponentButton { componentButtonStyle = style } -> style) <$> cbs
      )
      <$> xs
...
updateTicTacToe
  :: Maybe (T.Text, Bool) -> [ComponentActionRow] -> [ComponentActionRow]
updateTicTacToe Nothing _ =
  (\y ->
      ComponentActionRowButton
        $   (\x -> ComponentButton (T.pack $ "ttt " <> show x <> show y)
                                   False
...
updateTicTacToe (Just (tttxy, isFirst)) car
  | not (checkIsValid tttxy)
  = car
  | otherwise
  = (\(ComponentActionRowButton cbs) ->
      ComponentActionRowButton (changeIf <$> cbs)
    )
    <$> car
 where
  checkIsValid tttxy' =
    T.length tttxy' == 6 && all isDigit [T.index tttxy' 4, T.index tttxy' 5]
  getxy tttxy' = (T.index tttxy' 4, T.index tttxy' 5)
  (style, symbol) =
    if isFirst then (ButtonStyleSuccess, "[X]") else (ButtonStyleDanger, "[O]")
  changeIf cb@ComponentButton {..}
    | checkIsValid componentButtonCustomId
      && getxy tttxy
      == getxy componentButtonCustomId
    = cb { componentButtonDisabled = True
         , componentButtonStyle    = style
         , componentButtonLabel    = symbol
         }
    | otherwise
    = cb
  changeIf cb = cb
