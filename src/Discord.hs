...
runDiscord opts = do
  ...
  (gate , gateId ) <- liftIO $ startGatewayThread (Auth (discordToken opts))
                                                  (discordGatewayIntent opts)
                                                  cache
                                                  log
  ...
  let handle = DiscordHandle
        ...
        , discordHandleThreads      = [ HandleThreadIdLogger logId
  ...
  case resp of
    Left (RestCallInternalErrorCode c e1 e2) ->
      libError
        ...
        <> " "
        <> TE.decodeUtf8 e2
    Left (RestCallInternalHttpException e) ->
      libError ("HTTP Exception -  " <> T.pack (show e))
    Left (RestCallInternalNoParse _ _) ->
      libError "Couldn't parse GetCurrentUser"
    _ -> do
      me <- liftIO . runReaderT (try $ discordOnStart opts) $ handle
      case me of
        Left (e :: SomeException) -> libError
  ...
  loop = do
    ...
    case next of
      ...
      Right (Right event) -> do
        ...
        let action =
              if discordForkThreadForEvents opts then void . forkIO else id
        action $ do
          me <-
            liftIO . runReaderT (try $ discordOnEvent opts userEvent) $ handle
          case me of
            Left (e :: SomeException) -> writeChan
              (discordHandleLog handle)
              (  "eventhandler - crashed on ["
              <> T.pack (show userEvent)
              <> "] "
              <> "          with error: "
              <> T.pack (show e)
              )
    ...
    then pure (Left (RestCallErrorCode 400 "Library Stopped Working" ""))
      ...
      case resp of
        ...
        Left (RestCallInternalNoParse err dat) -> do
          ...
          pure
            (Left (RestCallErrorCode 400 "Library Parse Exception" formaterr))
...
sendCommand e = do
  ...
  case e of
    UpdateStatus opts -> liftIO $ writeIORef
      (gatewayHandleLastStatus (discordHandleGateway h))
      (Just opts)
    _ -> pure ()
...
startLogger handle logC = forkIO $ forever $ do
