...
connectionLoop auth intent gatewayHandle log = outerloop LoopStart
 where
  ...
  firstmessage state = case state of
  ...
  startconnectionpls first =
    runSecureClient "gateway.discord.gg"
                    443
                    ("/?v=" <> T.unpack apiVersion <> "&encoding=json")
      $ \conn -> do
 ...
 where
  ...
  loop      = do
    ...
    case eitherPayload :: Either ConnectionException GatewayReceivable of
      ...
      Right (InvalidSession retry) ->
        pure $ if retry then LoopReconnect else LoopStart
      ...
      Left (CloseRequest code str) -> case code of
        ...
        4014 -> do
          writeChan
            eventChan
            (Left
              (  GatewayExceptionUnexpected (Hello 0)
        ...
        _ -> do
          writeChan
            log
            (  "gateway - unknown websocket close code "
            <> T.pack (show code)
            <> " ["
...
sendableLoop
  :: Connection -> GatewayHandle -> SendablesData -> Chan T.Text -> IO ()
sendableLoop conn ghandle sendablesData _log = sendSysLoop
 ...
 -- writeChan _log ("gateway - sending " <> TE.decodeUtf8 (BL.toStrict (encode payload)))
    ...
    if not usersending
      then sendSysLoop
        ...
        act <- readIORef (gatewayHandleLastStatus ghandle)
        case act of
          Nothing   -> pure ()
 ...
 -- payload :: Either GatewaySendableInternal GatewaySendable
    payload <- race (readChan (gatewayHandleUserSendables ghandle))
