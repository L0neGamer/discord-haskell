...
instance ToJSON ChannelInviteOpts where
  toJSON ChannelInviteOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
...
instance ToJSON ModifyChannelOpts where
  toJSON ModifyChannelOpts {..} = object
    [ (name, val)
    | (name, Just val) <-
      ...
      , ( "default_auto_archive_duration"
        , toJSON <$> modifyChannelDefaultAutoArchive
        )
...
instance ToJSON ChannelPermissionsOptsType where
  toJSON t = case t of
    ChannelPermissionsOptsUser -> String "member"
    ChannelPermissionsOptsRole -> String "role"

instance ToJSON ChannelPermissionsOpts where
  toJSON (ChannelPermissionsOpts a d t) =
    object [("allow", toJSON a), ("deny", toJSON d), ("type", toJSON t)]
...
instance ToJSON StartThreadOpts where
  toJSON StartThreadOpts {..} = object
    [ (name, value)
    | (name, Just value) <-
      [ ("name"                 , pure (toJSON startThreadName))
...
instance ToJSON StartThreadNoMessageOpts where
  toJSON StartThreadNoMessageOpts {..} = object
    [ (name, value)
    | (name, Just value) <-
      [ ("name", pure (toJSON (startThreadName startThreadNoMessageBaseOpts)))
      , ( "auto_archive_duration"
        , toJSON <$> startThreadAutoArchive startThreadNoMessageBaseOpts
        )
      , ( "rate_limit_per_user"
        , toJSON <$> startThreadRateLimit startThreadNoMessageBaseOpts
        )
      , ("type"     , pure (toJSON startThreadNoMessageType))
...
instance FromJSON ListThreads where
  parseJSON = withObject "ListThreads" $ \o ->
    ListThreads <$> o .: "threads" <*> o .: "members" <*> o .: "has_more"
...
channelJsonRequest c = case c of
  (GetChannel chan) -> Get (channels // chan) mempty

  (ModifyChannel chan patch) ->
    Patch (channels // chan) (pure (R.ReqBodyJson patch)) mempty

  (DeleteChannel chan) -> Delete (channels // chan) mempty
  ...
  (CreateMessageDetailed chan msgOpts) ->
    let
      fileUpload = messageDetailedFile msgOpts
      filePart =
        (case fileUpload of
            Nothing -> []
            Just f ->
          ...
          ++ join
               (maybe [] (maybeEmbed . Just <$>) (messageDetailedEmbeds msgOpts)
               )
      ...
      payloadData =
        object
          ...
          ++ [ name .= value
             | (name, Just value) <-
               [ ( "embeds"
                 , toJSON . (createEmbed <$>) <$> messageDetailedEmbeds msgOpts
                 )
               , ( "allowed_mentions"
                 , toJSON <$> messageDetailedAllowedMentions msgOpts
                 )
               , ( "message_reference"
                 , toJSON <$> messageDetailedReference msgOpts
                 )
    ...
    in
      Post (channels // chan /: "messages") body mempty
  ...
  (DeleteUserReaction (chan, msgid) uID emoji) ->
    let e = cleanupEmoji emoji
    in  Delete
          (channels // chan /: "messages" // msgid /: "reactions" /: e // uID)
          mempty

  (DeleteSingleReaction (chan, msgid) emoji) ->
    let e = cleanupEmoji emoji
    in  Delete (channels // chan /: "messages" // msgid /: "reactions" /: e)
               mempty
  ...
  (GetReactions (chan, msgid) emoji (n, timing)) ->
    ...
    in  Get (channels // chan /: "messages" // msgid /: "reactions" /: e)
            options
  ...
  (EditMessage (chan, msg) msgOpts) ->
    let
      ...
      body        = R.reqBodyMultipart (payloadPart : filePart)
    in
      Patch (channels // chan /: "messages" // msg) body mempty
  ...
  (EditChannelPermissions chan perm patch) -> Put
    (channels // chan /: "permissions" // perm)
    (R.ReqBodyJson patch)
    mempty

  (GetChannelInvites chan) -> Get (channels // chan /: "invites") mempty
  ...
  (GetPinnedMessages chan) -> Get (channels // chan /: "pins") mempty
  ...
  (GroupDMAddRecipient chan (GroupDMAddRecipientOpts uid nick tok)) -> Put
    (channels // chan // chan /: "recipients" // uid)
    (R.ReqBodyJson
      (object [("access_token", toJSON tok), ("nick", toJSON nick)])
    )
    mempty
  ...
  (StartThreadFromMessage chan mid sto) -> Post
    (channels // chan /: "messages" // mid /: "threads")
    (pure $ R.ReqBodyJson $ toJSON sto)
    mempty

  (StartThreadNoMessage chan sto) -> Post
    (channels // chan /: "messages" /: "threads")
    (pure $ R.ReqBodyJson $ toJSON sto)
    mempty

  (JoinThread chan) ->
    Put (channels // chan /: "thread-members" /: "@me") R.NoReqBody mempty

  (AddThreadMember chan uid) ->
    Put (channels // chan /: "thread-members" // uid) R.NoReqBody mempty

  (LeaveThread chan) ->
    Delete (channels // chan /: "thread-members" /: "@me") mempty

  (RemoveThreadMember chan uid) ->
    Delete (channels // chan /: "thread-members" // uid) mempty

  (GetThreadMember chan uid) ->
    Get (channels // chan /: "thread-members" // uid) mempty

  (ListThreadMembers chan) -> Get (channels // chan /: "thread-members") mempty

  (ListPublicArchivedThreads chan (time, lim)) -> Get
    (channels // chan /: "threads" /: "archived" /: "public")
    (maybe mempty ("limit" R.=:) lim <> maybe mempty ("before" R.=:) time)

  (ListPrivateArchivedThreads chan (time, lim)) -> Get
    (channels // chan /: "threads" /: "archived" /: "private")
    (maybe mempty ("limit" R.=:) lim <> maybe mempty ("before" R.=:) time)

  (ListJoinedPrivateArchivedThreads chan (time, lim)) -> Get
    (  channels
    // chan
    /: "users"
    /: "@me"
