...
newtype CurrentUserAvatar = CurrentUserAvatar T.Text
  deriving (Show, Read, Eq, Ord)

parseCurrentUserAvatar :: B.ByteString -> Either T.Text CurrentUserAvatar
parseCurrentUserAvatar bs = case decodeImage bs of
  Left e -> Left (T.pack e)
  Right im ->
    Right $ CurrentUserAvatar $ "data:image/png;base64," <> TE.decodeUtf8
      (B64.encode (BL.toStrict (encodePng (convertRGBA8 im))))
...
userJsonRequest c = case c of
  ...
  (ModifyCurrentUser name (CurrentUserAvatar im)) -> Patch
    (users /: "@me")
    (pure (R.ReqBodyJson (object ["username" .= name, "avatar" .= im])))
    mempty
