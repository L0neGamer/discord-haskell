...
hexToDiscordColor hex =
  let (r, g, b) = fromMaybe (0, 0, 0) $ hexToRGB hex in DiscordColorRGB r g b
...
convertToRGB i =
  DiscordColorRGB (div i (256 * 256) .&. 255) (div i 256 .&. 255) (i .&. 255)
...
instance InternalDiscordEnum DiscordColor where
  ...
  discordTypeTable      = map (\d -> (fromDiscordType d, d))
                              (makeTable discordTypeStartValue)
   where
    makeTable :: Data b => b -> [b]
    makeTable t = map (fromConstrB (fromConstr (toConstr (0 :: Int))))
                      (dataTypeConstrs $ dataTypeOf t)
...
instance FromJSON DiscordColor where
  parseJSON = withScientific
   ...
   where
    maybeInt i | fromIntegral (round i) == i = Just $ round i
               | otherwise                   = Nothing
