...
instance ToJSON ComponentActionRow where
  toJSON (ComponentActionRowButton bs) =
    object [("type", Number 1), ("components", toJSON bs)]
  toJSON (ComponentActionRowSelectMenu bs) =
    object [("type", Number 1), ("components", toJSON [bs])]
...
mkButton label customId =
  ComponentButton customId False ButtonStyleSecondary (Just label) Nothing
...
instance ToJSON ComponentButton where
  toJSON ComponentButtonUrl {..} = object
    [ (name, value)
    | (name, Just value) <-
      [ ("type"    , Just $ Number 2)
      , ("style"   , Just $ Number 5)
...
mkSelectMenu customId sos =
  ComponentSelectMenu customId False sos Nothing Nothing Nothing
...
instance ToJSON ComponentSelectMenu where
  toJSON ComponentSelectMenu {..} = object
    [ (name, value)
    | (name, Just value) <-
      [ ("type"       , Just $ Number 3)
...
instance ToJSON ComponentTextInput where
  toJSON ComponentTextInput {..} = object
    [ (name, value)
    | (name, Just value) <-
      [ ("type"       , Just $ Number 4)
...
mkComponentTextInput cid label =
  ComponentTextInput cid False label Nothing Nothing True "" ""
