    ...
    case op of
      0 -> do
        ...
        case ejson of
          ...
          _other ->
            Dispatch
                (InternalUnknownEvent "Dispatch payload wasn't an object" o)
              <$> o
              .:  "s"
...
instance ToJSON GatewaySendableInternal where
  toJSON (Heartbeat i) =
    object ["op" .= (1 :: Int), "d" .= if i <= 0 then "null" else show i]
  ...
  toJSON (Resume token session seqId) = object
    [ "op" .= (6 :: Int)
    , "d" .= object
      ["token" .= authToken token, "session_id" .= session, "seq" .= seqId]
    ]
...
instance ToJSON GatewaySendable where
  ...
  toJSON (RequestGuildMembers (RequestGuildMembersOpts guild query limit)) =
    object
      [ "op" .= (8 :: Int)
      , "d" .= object ["guild_id" .= guild, "query" .= query, "limit" .= limit]
      ]
